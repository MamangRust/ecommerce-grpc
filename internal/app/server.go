...
func (s *Server) initResilience() *middlewares.ResilienceInterceptor {
	...
	circuitBreaker := resilience.NewCircuitBreaker(5, 30, s.Logger)
	requestLimiter := resilience.NewRequestLimiter(100, s.Logger)
...
func initRedisServer(ctx context.Context, logger logger.LoggerInterface) (*redis.Client, error) {
	...
	logger.Info("Redis connection established",
		zap.String("addr", fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST"), viper.GetString("REDIS_PORT"))),
		zap.Int("db", viper.GetInt("REDIS_DB")),
	)
